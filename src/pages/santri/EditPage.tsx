...
export default function EditPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const [santriRes, rumahQuranRes] = await Promise.all([
          api.get<Santri[]>("santri", {
            select: "*",
        ...
        const santri = santriRes.data?.[0];
        if (!santri) {
          setError("Santri not found");
          return;
        }
        ...
        setFormData({
          ...
          rumah_quran_id: santri.rumah_quran_id?.toString() || "",
          institution_origin: santri.institution_origin || "",
          enrollment_status: santri.enrollment_status || "active",
          enrollment_date: santri.enrollment_date || "",
          graduation_status: santri.graduation_status || "not_graduated",
          graduation_date: santri.graduation_date || "",
        });
      } catch (err: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error } = await api.update("santri", updateData, {
        id: `eq.${id}`,
      });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h2 className="text-2xl font-bold tracking-tight">Edit Santri</h2>
